...
const link: CSSProperties = { background: 'none', border: 'none', textAlign: 'left', color: '#8790A6', fontSize: 14, padding: 0 }
const head: CSSProperties = { fontWeight: 600, color: '#fff', fontSize: 14, marginBottom: 15 }

export default function Footer({ app }: { app: AppCtx }) {
  return (
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto', padding: 'clamp(44px,5vw,68px) clamp(16px,4vw,40px) 0', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 40, justifyContent: 'space-between', paddingBottom: 44 }}>
          <div style={{ flex: '1 1 260px', maxWidth: 340 }}>
            ...
            <p style={{ fontSize: 14, lineHeight: 1.6, color: '#8790A6', margin: '0 0 18px' }}>Агентские закупки электроники за рубежом по «белой» схеме. Комиссия 2–3%, оплата по СБП, документы для ИП.</p>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 44 }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                <button data-route="catalog" onClick={app.onNav} className="ta-footlink" style={link}>Каталог</button>
                <button data-route="how" onClick={app.onNav} className="ta-footlink" style={link}>Как это работает</button>
                <button data-route="orders" onClick={app.onNav} className="ta-footlink" style={link}>Кабинет партнёра</button>
                <button data-route="new" onClick={app.onNav} className="ta-footlink" style={link}>Новый заказ</button>
              </div>
            </div>
            <div>
              <div style={head}>Правовое</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                <button data-route="how" onClick={app.onNav} className="ta-footlink" style={link}>Агентский договор</button>
                <button data-route="how" onClick={app.onNav} className="ta-footlink" style={link}>Публичная оферта</button>
                <button data-route="how" onClick={app.onNav} className="ta-footlink" style={link}>Политика конфиденциальности</button>
              </div>
            </div>
            <div>
        ...
        <div style={{ borderTop: '1px solid #1C2440', padding: '22px 0 26px', display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', justifyContent: 'space-between', fontSize: 12.5, color: '#6E7691' }}>
          ...
          <span style={{ fontFamily: "'JetBrains Mono',monospace" }}>Легально · Белая схема · Комиссия 2–3%</span>
        </div>
      </div>
