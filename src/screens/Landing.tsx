...
import { CATEGORY_LIST, categoryMeta, decorateProduct, CALC_PRESETS, HOW_STEPS, Icon } from '../lib/techagent'
...
export default function Landing() {
  ...
  const calc = calculateOrder(calcPrice, false)

  const catTiles = useMemo(() => CATEGORY_LIST.map((c) => {
    const meta = categoryMeta(c)
    const count = products.filter((p) => p.category === c).length
    return { key: c, label: meta.label, accent: meta.accent, tint: meta.tint, count: count + ' поз.', icon: <Icon name={meta.glyph} size={26} color={meta.accent} sw={1.7} /> }
  }), [])

  const popular = useMemo(() => {
    const picks = ['Смартфоны', 'Ноутбуки', 'Планшеты', 'Наушники', 'Игровые консоли', 'Часы']
    return picks.map((cat) => products.find((p) => p.category === cat && p.inStock)).filter((p): p is typeof products[number] => !!p).slice(0, 4).map(decorateProduct)
  }, [])
    ...
    <section>
      ...
      <div style={{ position: 'relative', maxWidth: 1220, margin: '0 auto', padding: 'clamp(28px,5vw,64px) clamp(16px,4vw,40px) clamp(8px,2vw,20px)', background: 'radial-gradient(820px 420px at 82% -6%,rgba(27,68,245,.10),transparent 65%),radial-gradient(640px 360px at 0% 42%,rgba(251,44,54,.05),transparent 62%)' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(30px,5vw,64px)', alignItems: 'center' }}>
          <div style={{ flex: '1 1 440px', minWidth: 0 }}>
            ...
            <p style={{ fontSize: 'clamp(16px,1.6vw,18.5px)', lineHeight: 1.55, color: '#5B647A', maxWidth: 540, margin: '0 0 30px' }}>Партнёры-ИП закупают через нас оригинальную электронику за рубежом и передают клиентам по официальной схеме. Ваша цена — стоимость товара плюс честная комиссия <b style={{ color: '#0B1020' }}>3%</b>. Оплата по СБП, чек и документы включены.</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 26 }}>
              <button onClick={() => navigate('/catalog')} className="ta-btn-primary ta-lift" style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: '#1B44F5', color: '#fff', border: 'none', borderRadius: 13, padding: '16px 24px', fontWeight: 600, fontSize: 16, boxShadow: '0 10px 26px rgba(27,68,245,.3)' }}>Открыть каталог {arrow}</button>
              <button onClick={() => navigate('/how-it-works')} className="ta-outline" style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: '#fff', color: '#0B1020', border: '1px solid #E7E9F2', borderRadius: 13, padding: '16px 24px', fontWeight: 600, fontSize: 16 }}>Как это работает</button>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 13.5, fontWeight: 500, color: '#3A4256' }}><Icon name="shield" size={17} color="#12B981" />Официальный договор</span>
              <span style={{ width: 1, height: 16, background: '#E0E3EE', alignSelf: 'center' }} />
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 13.5, fontWeight: 500, color: '#3A4256' }}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#1B44F5" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M8 20V5h5a4 4 0 0 1 0 8H6M6 16h7" /></svg>Комиссия 3%</span>
              <span style={{ width: 1, height: 16, background: '#E0E3EE', alignSelf: 'center' }} />
          ...
          <div style={{ flex: '1 1 380px', minWidth: 0, display: 'flex', justifyContent: 'center' }}>
            <div style={{ width: '100%', maxWidth: 440, background: '#fff', border: '1px solid #E7E9F2', borderRadius: 24, boxShadow: '0 30px 70px rgba(11,16,32,.1)', padding: 'clamp(20px,3vw,28px)', position: 'relative', animation: 'ta-fade .5s ease both' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                ...
                <span style={{ font: "600 12px/1 'JetBrains Mono',monospace", color: '#0B7A55', background: '#E4F8F0', padding: '6px 9px', borderRadius: 8 }}>3%</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F5F6FB', border: '1px solid #E7E9F2', borderRadius: 14, padding: '10px 12px', marginBottom: 12 }}>
                <button onClick={() => setCalc(calcPrice - 5000)} className="ta-outline" style={{ width: 38, height: 38, flex: 'none', borderRadius: 10, border: '1px solid #E7E9F2', background: '#fff', fontSize: 22, fontWeight: 600, color: '#3A4256', display: 'grid', placeItems: 'center', lineHeight: 1 }}>−</button>
                <div style={{ flex: 1, textAlign: 'center', fontFamily: "'Unbounded',sans-serif", fontWeight: 700, fontSize: 'clamp(22px,4vw,28px)', letterSpacing: '-.02em' }}>{formatPrice(calcPrice)}</div>
                <button onClick={() => setCalc(calcPrice + 5000)} className="ta-outline" style={{ width: 38, height: 38, flex: 'none', borderRadius: 10, border: '1px solid #E7E9F2', background: '#fff', fontSize: 22, fontWeight: 600, color: '#3A4256', display: 'grid', placeItems: 'center', lineHeight: 1 }}>+</button>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 18 }}>
                {CALC_PRESETS.map((pr) => {
                  const active = pr.v === calcPrice
                  return <button key={pr.v} onClick={() => setCalc(pr.v)} style={{ border: 'none', borderRadius: 999, padding: '8px 12px', fontWeight: 600, fontSize: 12.5, transition: '.14s', background: active ? '#1B44F5' : '#F2F4FB', color: active ? '#fff' : '#3A4256' }}>{pr.label}</button>
                })}
              </div>
              <div style={{ borderTop: '1px dashed #E0E3EE', paddingTop: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 11 }}><span style={{ fontSize: 14, color: '#5B647A' }}>Стоимость товара</span><span style={{ fontWeight: 600, fontSize: 14.5 }}>{formatPrice(calcPrice)}</span></div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}><span style={{ fontSize: 14, color: '#5B647A', display: 'inline-flex', alignItems: 'center', gap: 6 }}>Комиссия агента <span style={{ font: "600 11px/1 'JetBrains Mono',monospace", color: '#1B44F5', background: '#EDF0FF', padding: '3px 6px', borderRadius: 6 }}>3%</span></span><span style={{ fontWeight: 600, fontSize: 14.5, color: '#1B44F5' }}>+{formatPrice(calc.commission)}</span></div>
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', background: '#0B1020', borderRadius: 14, padding: '15px 17px' }}><span style={{ fontSize: 13.5, color: '#AEB6C9', fontWeight: 500 }}>Итого к оплате</span><span style={{ fontFamily: "'Unbounded',sans-serif", fontWeight: 700, fontSize: 'clamp(20px,3.6vw,25px)', color: '#fff', letterSpacing: '-.02em' }}>{formatPrice(calc.totalCost)}</span></div>
              </div>
              <p style={{ fontSize: 11.5, color: '#8891A5', margin: '12px 2px 16px', lineHeight: 1.5 }}>Доставка и таможенные платежи уже включены в стоимость товара за рубежом.</p>
              <button onClick={() => navigate('/dashboard/orders/new')} className="ta-btn-primary" style={{ width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#1B44F5', color: '#fff', border: 'none', borderRadius: 13, padding: 15, fontWeight: 600, fontSize: 15.5, boxShadow: '0 8px 20px rgba(27,68,245,.26)' }}>Оформить заказ по СБП</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto', padding: 'clamp(20px,3vw,34px) clamp(16px,4vw,40px)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 1, background: '#E7E9F2', border: '1px solid #E7E9F2', borderRadius: 20, overflow: 'hidden' }}>
          {[[products.length + '+', 'товаров в каталоге', undefined], ['3%', 'комиссия агента', '#1B44F5'], ['9 стран', 'закупки под ключ', undefined], ['5–7 дней', 'доставка в РФ', undefined]].map(([big, small, color], i) => (
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto', padding: 'clamp(30px,4vw,54px) clamp(16px,4vw,40px)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 26 }}>
          ...
          <button onClick={() => navigate('/catalog')} className="ta-ghost" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'none', border: 'none', color: '#1B44F5', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>Весь каталог {arrowSm}</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(158px,1fr))', gap: 14 }}>
          {catTiles.map((c) => (
            <button key={c.key} onClick={() => navigate(`/catalog?cat=${encodeURIComponent(c.key)}`)} className="ta-tile" style={{ textAlign: 'left', background: '#fff', border: '1px solid #E7E9F2', borderRadius: 18, padding: '20px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto', padding: 'clamp(20px,3vw,34px) clamp(16px,4vw,40px) clamp(30px,4vw,54px)' }}>
        <div style={{ background: '#fff', border: '1px solid #E7E9F2', borderRadius: 24, padding: 'clamp(24px,4vw,44px)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 30 }}>
            ...
            <button onClick={() => navigate('/how-it-works')} className="ta-ghost" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'none', border: 'none', color: '#1B44F5', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>Подробнее {arrowSm}</button>
          </div>
            ...
            <div style={{ flex: '1 1 380px', minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
                {['Агентский отчёт', 'Кассовый чек', 'Гарантийный талон'].map((t) => (
                  <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#151B33', border: '1px solid #232B47', borderRadius: 12, padding: '11px 15px', fontWeight: 500, fontSize: 13.5 }}><Icon name="doc" size={16} color="#8FA9FF" />{t}</span>
                ))}
              </div>
            </div>
            <div style={{ flex: '1 1 320px', minWidth: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 14 }}>
              {[['Агентский договор', 'Договор поручения с каждым ИП', 'rgba(143,169,255,.14)', '#8FA9FF'], ['Чек и документы', 'Закрывающие для бухгалтерии', 'rgba(94,227,180,.14)', '#5EE3B4'], ['Оплата по СБП', 'На расчётный счёт, прозрачно', 'rgba(251,44,54,.14)', '#FF7A81'], ['Гарантия и трек', 'Статус заказа в кабинете', 'rgba(143,169,255,.14)', '#8FA9FF']].map(([t, d, bg, stroke], i) => (
                <div key={i} style={{ background: '#151B33', border: '1px solid #232B47', borderRadius: 18, padding: 22 }}><span style={{ width: 42, height: 42, borderRadius: 11, background: bg, display: 'grid', placeItems: 'center', marginBottom: 14 }}><Icon name="shield" size={20} color={stroke as string} /></span><div style={{ fontWeight: 600, fontSize: 15, marginBottom: 6 }}>{t}</div><div style={{ fontSize: 13, color: '#AEB6C9', lineHeight: 1.5 }}>{d}</div></div>
              ))}
            </div>
          </div>
        </div>
      ...
      {popular.length > 0 && (
        <div style={{ maxWidth: 1220, margin: '0 auto', padding: 'clamp(36px,5vw,64px) clamp(16px,4vw,40px)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 26 }}>
            <div><div style={{ font: "600 12px/1 'JetBrains Mono',monospace", color: '#1B44F5', letterSpacing: '.08em', marginBottom: 12 }}>/ ИЗ КАТАЛОГА</div><h2 style={{ fontFamily: "'Unbounded',sans-serif", fontWeight: 700, fontSize: 'clamp(1.7rem,3.4vw,2.6rem)', letterSpacing: '-.02em', margin: 0 }}>Популярные категории</h2></div>
            <button onClick={() => navigate('/catalog')} className="ta-ghost" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'none', border: 'none', color: '#1B44F5', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>Смотреть всё {arrowSm}</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 16 }}>
            {popular.map((p) => <ProductCard key={p.id} p={p} onClick={() => navigate(`/catalog/${p.id}`)} />)}
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto', padding: '0 clamp(16px,4vw,40px) clamp(40px,6vw,72px)' }}>
        <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(120deg,#1B44F5,#0E2FCC)', borderRadius: 28, padding: 'clamp(34px,5vw,60px)', color: '#fff' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', flexWrap: 'wrap', gap: 24, alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ flex: '1 1 380px' }}><h2 style={{ fontFamily: "'Unbounded',sans-serif", fontWeight: 700, fontSize: 'clamp(1.8rem,3.6vw,2.8rem)', letterSpacing: '-.025em', lineHeight: 1.05, margin: '0 0 12px' }}>Готовы стать партнёром?</h2><p style={{ fontSize: 16.5, color: '#D5DDFF', margin: 0, maxWidth: 440, lineHeight: 1.5 }}>Зарегистрируйтесь как ИП-партнёр или откройте каталог, чтобы увидеть итоговую цену за пару секунд.</p></div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
              <button onClick={() => navigate('/register')} className="ta-lift" style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: '#fff', color: '#0B1020', border: 'none', borderRadius: 13, padding: '16px 24px', fontWeight: 600, fontSize: 16 }}>Стать партнёром {arrow}</button>
              <button onClick={() => navigate('/catalog')} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: 'rgba(255,255,255,.14)', color: '#fff', border: '1px solid rgba(255,255,255,.35)', borderRadius: 13, padding: '16px 24px', fontWeight: 600, fontSize: 16 }}>Открыть каталог</button>
            </div>
          </div>
        </div>
      </div>
